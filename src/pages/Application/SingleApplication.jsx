...
const SingleApplication = () => {
  ...
  const query = `${params.id}?JobStatus=${jobStatus}&Name=${searchData}`;

  useEffect(() => {
    if (userInfo) {
      dispatch(singleApplication({ data: query, token: currentUser.Token }));
    }
  }, [userInfo, params, jobStatus, searchData, page, pageSize]);
  ...
  return (
    <>
      <PageHeading
        ...
        subTitle={
          application && application.Values && application.Values.length > 0 ? (
            <>
              {application.Values.length} Applicant(s) for{" "}
